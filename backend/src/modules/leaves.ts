...
router.get("/pending", requireAuth, requireRole("ADMIN", "HR", "MANAGER"), async (_req: Request, res: Response) => {
  const requests = await prisma.leaveRequest.findMany({ where: { status: "PENDING" }, include: { leaveType: true } });
  res.json({ requests });
});

router.patch("/:id/approve", requireAuth, requireRole("ADMIN", "HR", "MANAGER"), async (req: Request & { user?: { id: string } }, res: Response) => {
...
router.patch("/:id/reject", requireAuth, requireRole("ADMIN", "HR", "MANAGER"), async (req: Request & { user?: { id: string } }, res: Response) => {
